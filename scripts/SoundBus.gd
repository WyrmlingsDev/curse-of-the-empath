...
func _queue_sound_at(sound_name: String, sound_stream: AudioStream, _position: Vector2, _start_from: float) -> void:
	if currently_playing.has(sound_name):
		return
...
func _queue_sound_volume(sound_name: String, sound_stream: AudioStream, _position: Vector2, _volume: float) -> void:
	if currently_playing.has(sound_name):
		return
	...
	sound_instance.position = _position
	sound_instance.stream = sound_stream
	sound_instance.volume_db = _volume
	sound_instance.connect("finished", Callable(self, "_on_sound_finished").bind(sound_name, sound_instance))
	add_child(sound_instance)
	sound_instance.play()
	currently_playing[sound_name] = sound_instance
